...
```{r}
iris_raw <- read.csv("/project/obds/shared/resources/2_r/r_tidyverse/iris.csv")
```
...
iris_raw %>%
  group_by(species) %>%
  summarise(sepal_length_mean= mean(sepal_length), sepal_length_median = median(sepal_length), sepal_length_min = min(sepal_length), sepal_length_max = max(sepal_length))          
```
...
ggplot(iris_for_ggplot,aes(x=sepal_area, fill = species) ) +
  geom_histogram() + facet_wrap(~species, ncol =1, nrow = 3)
```
...
```{r}
library(readxl)
excel_sheets("/t1-data/project/obds/ldenly/ldenly/git/OBDS_May_2022/Tidyverse/EH2011.xlsx")
exprs_assay <- read_excel("/t1-data/project/obds/ldenly/ldenly/git/OBDS_May_2022/Tidyverse/EH2011.xlsx", sheet = "exprs")
...
```{r}
exprs_assay_long <- exprs_assay %>% pivot_longer(cols = starts_with("TCGA"))
...
```{r}
library(ggplot2)
ggplot(exprs_assay_long, aes(x=value)) + geom_histogram() + facet_wrap(~gene)
...
library(readxl)
excel_sheets("/t1-data/project/obds/ldenly/ldenly/git/OBDS_May_2022/Tidyverse/EH2011.xlsx")
sample_info <- read_excel("/project/obds/ldenly/ldenly/git/OBDS_May_2022/Tidyverse/EH2011.xlsx", sheet = "sample_info")
sample_info
```
