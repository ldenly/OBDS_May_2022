...
ggplot(data = x_dataframe, mapping = aes(x=x_column)) + geom_histogram()
```
...
ggplot(data = x_dataframe, mapping = aes(x=x_column)) +
  geom_histogram() + geom_vline(xintercept = mean(x), color = "red") +
...
```{r}
x_a_lot <- rnorm(1000000, mean =10, sd=5)
x_a_lot_dataframe <- data.frame(x_column = x_a_lot)
ggplot(data = x_a_lot_dataframe, mapping = aes(x=x_column)) +
  geom_histogram() + geom_vline(xintercept = mean(x), color = "red") +
  geom_vline(xintercept = median(x), color = "red", linetype = "dashed") +
  geom_vline(xintercept=mean(x) + sd(x) * c(-1, 1), color = "blue", linetype = "dashed") +
  geom_vline(xintercept=median(x) + mad(x)* c(-1, 1), color = "blue", linetype = "dashed")
...
```{r}
dnorm_v <- dnorm(x =seq(-5,5, by=0.1), mean =0, sd=1)
gg_frame <- data.frame(value = dnorm_v, quantile=seq(-5,5, by=0.1))
head(gg_frame)
gg_frame %>% ggplot(aes(x=quantile, y=value))+geom_point()
```
...
```{r}
1-pnorm(q= seq(2,-2,by=-0.1), mean=0, sd=1)
```
...
```{r}
gene_index <- 1
gene_values <- logcounts_matrix[1,]
gene_values
cell_infection <- cell_info$Infection
cell_infection
test_data <- data.frame("Values" = gene_values, "Infection" = cell_infection)
head(test_data)
out <- t.test(Values ~ Infection, data = test_data)
str(out)
out$p.value
```
...
t_test_row <- function(index,matrix, metadata) {
  ...
  data_frame <- data.frame("Values" = gene_values, "Infection" = cell_infection)
  out <- t.test(Values ~ Infection, data = data_frame)
  return(out$p.value)
}
t_test_row(index = 1, matrix = logcounts_matrix, metadata = cell_info)
t_test_row(index = 2, matrix = logcounts_matrix, metadata = cell_info)
```
...
```{r}
t_test_df <- data.frame("p_values" = t_test_pvalues)
ggplot(t_test_df, aes(x=p_values)) + geom_histogram()
```
...
```{r}
result_bh <- p.adjust(t_test_df$p_values, method = "BH")
table(result_bh < 0.05)
table(t_test_df$p_values <0.05)
```
...
filter(gene_table, gene_id == "ENSG00000131203")
```
