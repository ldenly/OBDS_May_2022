...
```{r}
Ebox <- "CANNTG"
countPattern(pattern = Ebox, subject = chrY, fixed = FALSE) #fixed false means N in the pattern can match any sequence
```
...
```{r}
mart <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", "hsapiens_gene_ensembl", host = "https://may2021.archive.ensembl.org"
)
mart
```
...
getBM(c("hgnc_symbol", "chromosome_name",
 "start_position",
  "end_position",
  "strand"), 
 filters = "hgnc_symbol", values = human_symbols, mart = mart)
...
assays(demo_se)$badname
colData(demo_se)$score <- c(100,200)
rowData(demo_se)
mcols(demo_se)
metadata(demo_se) <- list(experiment_name = "Demo")
...
```{r}
assay_counts <- read_csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv")
assay_counts %>% column_to_rownames(var = "...1") -> assay_counts
assay_counts <- data.frame(assay_counts, row.names = )
head(assay_counts)
dim(assay_counts)
```
...
```{r}
sample_metadata <- data.frame(read_csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv"))
head(sample_metadata)
```

```{r}
gene_metadata <- data.frame(read_csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv"))
head(gene_metadata)
```
...
```{r}
demo_deseq <- as('DESeqDataSet', demo_se)
demo_deseq
```
